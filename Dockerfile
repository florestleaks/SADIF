...
ENV VIRTUAL_ENV /opt/sadif-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/sadif/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/sadif/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/sadif/ && touch src/sadif/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/sadif/
